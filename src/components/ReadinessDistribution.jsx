  ...
  useEffect(() => {
    async function fetchDistribution() {
      try {
        ...
        const prHistory = history.filter(
          (entry) =>
            entry.branch &&
            entry.branch !== "main" &&
            entry.branch !== "master" &&
        ...
        const mainHistory = history.filter(
          (entry) => entry.branch === "main" || entry.branch === "master"
        );
    ...
    <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-gray-800 rounded p-3">
          ...
          <div className="text-xl font-bold text-red-400">{stats.failing}</div>
        </div>
        <div className="bg-gray-800 rounded p-3">
          ...
          {Object.entries(bins).map(([range, data]) => (
            <div key={range} className="flex-1 text-center mx-1">
              ...
              <div className="text-xs text-gray-500 mt-1">
                {stats.total > 0
                  ? `${((data.count / stats.total) * 100).toFixed(0)}%`
                  : "0%"}
              </div>
            </div>
          ))}
